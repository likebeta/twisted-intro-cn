...
最后一条需要解释清楚。在Twisted中，reactor是[Singleton](http://en.wikipedia.org/wiki/Singleton_pattern)（也是一种模式），即在一个程序中只能有一个reactor，并且只要你引入它就相应地创建一个。上面引入的方式这是twisted默认使用的方法，当然了，twisted还有其它可以引入reactor的方法。例如，可以使用twisted.internet.pollreactor中的系统调用来[poll](http://www.makelinux.net/ldd3/chp-6-sect-3.shtml)来代替select方法。
...
下面是使用 pollreactor重写上上面的程序，可以在[basic-twisted/simple-poll.py](http://github.com/jdavisp3/twisted-intro/blob/master/basic-twisted/simple-poll.py)文件中找到中找到：
```python
