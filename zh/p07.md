...
下面是我们第一人使用deferred的例子[twisted-deferred/defer-1.py](http://github.com/jdavisp3/twisted-intro/blob/master/twisted-deferred/defer-1.py):
```python
...
运行结果如下：
```
twisted.python.failure.Failure [Failure instance: 
    Traceback (failure with no frames): : I have failed. ]
No poetry for you.
```
